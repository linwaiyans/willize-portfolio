...
import { SolecialBanner, FallDetectionBanner } from "./ProjectBanners";
            ...
            <Reveal key={p.title} variant="scale" delay={i * 100}>
              <div className="card card-hover overflow-hidden flex flex-col h-full group">
                ...
                <div className="p-6 md:p-7 flex flex-col flex-1">
                  <p className="text-xs font-semibold mb-1 uppercase tracking-wider" style={{ color: "var(--accent)" }}>
                    {p.subtitle}
                  </p>
                  <h3 className="text-2xl font-black mb-1" style={{ color: "var(--foreground)" }}>{p.title}</h3>
                  <p className="text-base mb-4" style={{ color: "var(--foreground-muted)" }}>{p.tagline}</p>

                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] mb-5 font-mono" style={{ color: "var(--muted)" }}>
                    <span className="flex items-center gap-1.5"><Calendar size={11} /> {p.period}</span>
                    <span className="flex items-center gap-1.5"><MapPin size={11} /> {p.location}</span>
                  </div>

                  <ul className="space-y-2.5 mb-5 flex-1">
                    {p.highlights.map((h) => (
                      <li key={h} className="flex gap-3 text-sm" style={{ color: "var(--foreground-muted)" }}>
                  ...
                  <div className="space-y-3 pt-5 border-t" style={{ borderColor: "var(--border)" }}>
                    <div>
                      <p className="text-[10px] font-bold mb-2 tracking-widest uppercase" style={{ color: "var(--muted)" }}>Tech Stack</p>
                    ...
                    <div>
                      <p className="text-[10px] font-bold mb-2 tracking-widest uppercase" style={{ color: "var(--muted)" }}>Skills Developed</p>
                      <div className="flex flex-wrap gap-1.5">
                        {p.skills.map((s) => (
                          <span
                            key={s}
